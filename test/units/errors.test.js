...
import {
  dispatchables, nonDispatchables, nonPlainObjects, plainObjects
} from '../utils'
...
describe('assertAreFunctions()', () => {
  ...
  test('throws a TypeError if any items are not functions', () => {
    ...
    expect(
      assertAreFunctions.bind(null, [ 1 ])
    ).toThrow(TypeError)

    expect(
      assertAreFunctions.bind(null, [ {} ])
    ).toThrow(TypeError)

    expect(
      assertAreFunctions.bind(
        null, [ () => {}, () => {}, 'a', () => {} ]
      )
    ).toThrow(TypeError)
...
describe('assertIsPlainObject()', () => {
  ...
  test('throws a TypeError if the action is not a plain object', () => {
    ...
    nonPlainObjects.forEach(
      nonPlainObject =>
        expect(
          assertIsPlainObject.bind(null, nonPlainObject)
        ).toThrow(TypeError)
    )
    ...
    plainObjects.forEach(
      plainObject =>
        expect(
          assertIsPlainObject.bind(null, plainObject)
        ).not.toThrow()
    )
...
describe('assertIsValidAction()', () => {
  ...
  test('throws a TypeError if the action does not have a "type" property', () => {
    ...
    nonDispatchables.forEach(
      nonDispatchable => expect(
        assertIsValidAction.bind(null, nonDispatchable)
      ).toThrow(TypeError)
    )
  ...
  test('does not throw if the action is valid', () => {
    ...
    dispatchables.filter(dispatchable => typeof dispatchable !== 'function')
      .forEach(
        dispatchable => expect(
          assertIsValidAction.bind(null, dispatchable)
        ).not.toThrow()
      )
...
describe('assertIsValidActor()', () => {
  ...
  test('throws a TypeError if the actor is invalid', () => {
    ...
    nonDispatchables.forEach(
      nonDispatchable => expect(
        assertIsValidActor.bind(null, nonDispatchable)
      ).toThrow(TypeError)
    )
...
describe('assertIsValidNodeOption()', () => {
  ...
  test('throws an Error of the given option is not in the validOptions list', () => {
    ...
    expect(
      assertIsValidNodeOption.bind(null, validOptions, 'a')
    ).toThrow(Error)
...
describe('invalidAction()', () => {
  ...
  test('returns a string containing the detailed type of the invalid "type" property', () => {
    ...
    expect(invalidAction({ type: 1 })).toMatch(/received number/i)

    expect(invalidAction({ type: [] })).toMatch(/received array/i)

    expect(invalidAction({ type: null })).toMatch(/received null/i)

    expect(
      invalidAction({ type: new Map() })
    ).toMatch(/received complex object/i)

    expect(
      invalidAction({ type: Object.create(null) })
    ).toMatch(/received prototype-less object/i)
...
describe('invalidActor()', () => {
  ...
  test('returns a string containing the type of the invalid actor', () => {
    ...
    expect(invalidActor()).toMatch(/received undefined/i)
  ...
  test('returns a string containing the type of the invalid "type" property of the actor', () => {
    ...
    expect(
      invalidActor(actor)
    ).toMatch(/function with invalid "type" property - number/i)
...
describe('invalidNodeOptionKey()', () => {
  ...
  test('returns a string containing the passed key and a helpful error message', () => {
    ...
    expect(
      invalidNodeOptionKey('a')
    ).toMatch(/received invalid node option, "a".*valid options are/i)
...
describe('invalidNonPlainObject()', () => {
  ...
  test('returns a string containing the detailed type of the invalid action', () => {
    ...
    expect(
      invalidNonPlainObject(Object.create(null))
    ).toMatch(/received prototype-less object/i)
