...
As you've probably guessed, use `ecosystem.dehydrate()` to grab the initial state snapshot from the server. Here's an example using a typical server route handler:
