...
While you can use a TypeScript union and that works well enough, you can also use an atom factory to create completely different atoms. Of course, this approach might require manual param serializing ([`atom.getKeyHash()`](../api/classes/Atom#getkeyhash) to the rescue!) and potentially lots of manual typing to create the properly-typed atoms. But it _could_ be worth it.
