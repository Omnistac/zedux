...
Zedux was created in TypeScript from the ground up. Many Zedux APIs are designed specifically to work well with TypeScript. The [API Docs](../api/api-overview#types) document many of Zedux's more prominent types. But there are lots of TypeScript helpers and nuances not covered anywhere else in these docs.
...
```ts
const getUpperCase = <I extends AtomInstanceType<typeof exampleAtom>>(
  instance: I
) => {
  // given the exampleAtom above, TS knows this `.getState()` returns a string:
  return instance.getState().toUpperCase()
}
```
...
const getValue: {
  ...
  ecosystem.get(template as AnyAtomTemplate) // this cast is fine
}
```
...
Now no overloads are needed, at the cost of a little tuple ugliness. Of course this doesn't support passing an atom instance like the overloads example. But for functions that don't need that, this may be good enough.
