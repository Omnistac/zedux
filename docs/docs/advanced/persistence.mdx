...
There are a few ways to get a state snapshot of an entire ecosystem. For example, you could call `ecosystem.inspectInstances()` and map each instance to a value yourself. But ecosystems have a method that's specifically designed for persistence:
...
`ecosystem.dehydrate()` returns an object mapping atom instance keyHashes to their current value. But with transforms enabled, `.dehydrate()` does a little more magic.
