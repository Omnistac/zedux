...
```tsx live ecosystemId=plugin-atom-example resultVar=App
...
If the plugin creates its own ecosystem and you know the id, you can use [`getEcosystem()`](../api/utils/internal-store-utils#getecosystem) to retrieve the plugin's ecosystem anywhere. Then use that ecosystem to interact with its atoms like you would with any ecosystem.
