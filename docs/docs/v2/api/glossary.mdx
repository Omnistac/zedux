...
Examples of unrestricted injectors include [`injectEcosystem()`](./injectors/injectEcosystem.mdx), [`injectSelf()`](./injectors/injectSelf.mdx), and [`injectWhy()`](/not-done?path=./injectors/injectWhy.mdx).
