...
```ts
import { injectSignal } from '@zedux/react'
```
...
<Legend>
  ...
  <Item name="config">
    ...
    <Legend>
      <Item name="events">
        An object mapping event names to `As<MyType>` where `As` is Zedux's [`As` util](../utils/As.mdx) and `MyType` is the payload type of the event.

        This payload must always be passed when the event is sent. Specify `As<undefined>` to allow no payload.
      </Item>
      <Item name="reactive">
