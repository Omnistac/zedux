...
<Legend>
  <Item name="invalidate" suffix="event">
    ...
    [`injectPromise`](../injectors/injectPromise.mdx)'s `runOnInvalidate`
    option.
  </Item>
...
<Legend>
  ...
  <Item name="template">
    ...
    reference to the [atom template](./AtomTemplate.mdx) this instance was
    created from.
  </Item>
</Legend>
...
<Legend>
  <Item name="invalidate">
    ...
    To access this method inside the atom's state factory, use [`injectSelf()`](../injectors/injectSelf.mdx).

  </Item>
</Legend>

Atom instances also inherit the following methods from [`Signal`](./Signal.mdx#methods):
