...
Zedux now runs effect callbacks passed to `injectEffect` immediately after the atom finishes evaluating, rather than setting a timeout and running later. This is a big improvement when working with atoms synchronously, as before you couldn't be sure if manual subscriptions were registered yet when using an atom. Now you can.

However, this more "eager" effect running can lead to some subtle bugs when migrating. For example:

```ts
import { atom, injectStore, removeAllMeta, zeduxTypes } from '@zedux/stores'

const childAtom = atom('child', () => {
  const store = injectStore('child state')

  injectEffect(() => {
    // this subscription is now registered before the store can be composed in
    // other atoms:
    const subscription = store.subscribe({
      effects: ({ action }) => {
        // to fix, ignore the `prime` action:
        if (removeAllMeta(action).type === zeduxTypes.prime) {
          return
        }
      },
    })
...
const parentAtom = atom('parent', () => {
  const { store: childStore } = injectAtomInstance(childAtom)

  // Zedux dispatches the special "prime" action when this composed store is
  // first created. That action will now reach `childAtom`'s store's subscriber,
  // whereas in v1 it wouldn't.
  const store = injectStore(() => createStore({ child: childStore }))

  return store
})
```

To avoid this problem in the first place, it's a best practice to always check `action.type` inside store effects listeners.

This ordering change may also break effects that were expecting other synchronous code to run between atom initialization and the deferred effect. For example, we had some test code awaiting an already-resolved promise right after initializing an atom. That microtask was expected to run before the effect and in v2 it doesn't. The fix is generally to make sure the effect explicitly awaits asynchronous operations it expects to run first.
