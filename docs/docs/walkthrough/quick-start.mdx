...
<LiveEditor ecosystemId="quick-start/setting-state" resultVar="Greeting">{`
const greetingAtom = atom('greeting', 'Hello, world!')\n
...
<LiveEditor ecosystemId="quick-start/todos" resultVar="Todos">{`
...
<LiveEditor ecosystemId="big-graph" resultVar="Num">{`
const test = atom('test', num => {
  const { get } = injectAtomGetters()
  const store = injectStore(false, true)\n
  if (num < 47 && store.getState()) {
    get(test, [num + 1])\n
    if (!(num % 3)) get(test, [55])
  }\n
  injectEffect(() => {
    setTimeout(() => store.setState(true), 50)
  }, [])\n
  return num
})\n
const Num = () => {
  const val = useAtomValue(test, [0])
  return val
}
`}</LiveEditor>
