...
The atom is the basic unit in Zedux. Here we created an atom by giving it a key, `greeting`, and an initial value, `Hello, world!`.

But this atom is a mere shell. To make the magic happen, we need to use the atom. We can do this with hooks:
...
The first time we use the atom, Zedux creates a new "atom instance" loaded with stateful goodies. This atom instance contains a powerful store that holds its state.
...
- Zedux sees that the `greeting` atom has never been used before, so it creates an atom instance with a store whose initial state is `'Hello, world!'` and `useAtomState` returns that state to `<GreetingPreview>`
...
When we create an atom, Zedux waits until the atom is used (e.g. by a component or another atom) to create an atom instance + store. This means that when we pass a state factory to `atom()`, that function won't be called until Zedux instantiates the atom. When the atom is used for the first time, Zedux creates an atom instance and calls the state factory to produce the initial value of the instance's store.
...
- Atoms are the basic unit in Zedux.
...
const filteredTodosAtom = atom('filteredTodos', (isDone: boolean) => {
  ...
  return todos.filter(todo => todo.isDone === isDone).map(({ text }) => text)
})
