...
Let's see a live example of what we have so far.
...
Ecosystems essentially make all atom instances "global" to the whole component tree under an `<EcosystemProvider>`. So why would we need to provide certain instances to parts of the tree?
...
Let's continue learning how to integrate atoms with React. Next up: [React suspense](suspense).
