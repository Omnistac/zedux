...
In the quick start we learned how to use the `useAtomState` hook to get and update an atom instance's state from a React component. Calling the `setState` function returned from `useAtomState` triggers an "external update". But atoms can also update themselves.
...
This state can affect everything else in the component - what effects to run, what happens inside callbacks, and even what UI to return. Wouldn't it be amazing if we had the same capability inside atoms?

Tada! We do. In Zedux we can give an atom its own internal, local state using an injector:
...
This creates a "zero-config" store with `'initial state'` as its ... initial state. Whenever this store's state updates, Zedux will call this state factory again.

We can use the current state of the store exactly like we'd use local state in React - to run side effects, change callbacks, and even change the state we return from this atom.
...
We can create stores completely outside atoms using `createStore`:
...
`injectStore` has a function overload that allows us to create the store ourselves:
...
Remember that we said every atom instance has its own store? Zedux creates this store automatically by default. But now that we know how to create our own stores, we can tell Zedux that it doesn't need to create one for us.
...
By returning the store from the state factory, we essentially tell Zedux, "Don't create a store for me! I created my own. Use this one."
...
With our store exposed, we are now able to update it both internally and externally. Let's expose the store in our counter example from earlier to see this in action:
...
Try clicking the button to make the counter count faster. Our `secondsAtom` now controls its own state _and_ simultaneously allows dependents to update it.

## Multiple Stores

An atom state factory can inject any number of stores. But only one store can be returned. Having unexposed "internal" stores is completely fine. But what if we want to expose multiple stores?
...
To return multiple stores, we create a single "parent" store composed of all the stores we want to expose.
...
Now that we know a bit more about creating atoms, it's time to learn [what atom instances look like](atom-instances).
