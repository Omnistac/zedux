...
In [the atom instances walkthrough](atom-instances), we learned that every atom instance has an `activeState` "lifecycle":
...
In the example above, if `MyComponent` is the only dependent on the `lifecycleTest` atom instance, its ref count will be 1. If `MyComponent` is unmounted, the ref count will go to 0 and Zedux will set the instance's `activeState` to `'Stale'`.

When a Stale atom instance's ref count goes back up to 1, its `activeState` transitions back to `'Active'`.
...
But how do we destroy the instance?

### TTL

We learned about `ttl` in [the configuring atoms walkthrough](configuring-atoms). This is the simplest way to destroy atom instances.

There are 4 possible TTL logic flows:
...
Additionally, when you pass a function to [`atomApi.setTtl()`](configuring-atoms#apisetttl), Zedux will call that function when the instance goes Stale and then follow the appropriate logic flow for the returned value.
...
This flow is not recommended in most cases. Prefer using the graph. We'll look at ways to manually update the graph [shortly](#manual-graphing).

## Atom Selector Destruction

Atom Selectors don't have the same `activeState` lifecycle status as atom instances. But Atom Selector caches do have a ref count and a lifecycle of sorts.
...
### `selectorCache.destroyCache()`

We saw this briefly in [the selectors walkthrough](selectors).

```ts
ecosystem.selectorCache.destroyCache(myAtomSelector)
```
...
We just saw that by default Zedux always tries to bail out of destruction if a node's ref count is `> 0`. But we can change this behavior.

`instance.destroy()` takes a single optional boolean parameter - `force`. Simply pass `true` to force destruction, ignoring ref count:

```ts
instance.destroy(true)
```

We can do the same thing with selector caches. `selectorCache.destroyCache` also has an optional `force` parameter as its 3rd param:

```ts
ecosystem.selectorCache.destroyCache(
  myAtomSelector,
  ['param 1', 'param 2'],
  true
)
...
ecosystem.selectorCache.destroyCache(myAtomSelector, [], true)
```
...
We've learned how to use hooks, injectors, and Atom Getters to make Zedux update the dependency graph automatically. But thanks to ecosystems, you can work with atom instances completely outside of React or atoms themselves. Since Zedux can't automatically update the graph in this case, you might encounter a few unideal situations:
...
It's that simple! When you call `instance.addDependent()`, Zedux creates a new "pseudo-node" in the graph and draws an edge between the instance and the new pseudo-node. This dependent means the instance's ref count can never hit 0 until we call the cleanup function.
...
### `selectorCache.addDependent()`

Atom Selectors have the same capability.

```ts
const cleanup = ecosystem.selectorCache.addDependent(myAtomSelector)

// with params:
ecosystem.selectorCache.addDependent(myAtomSelector, ['param 1', 'param 2'])
...
// if the selector doesn't take params, pass an empty array:
ecosystem.selectorCache.addDependent(myAtomSelector, [], {
  operation: 'mySpecificReasonForUsingTheSelectorHere'
})
```
...
:::info important
Unlike force-destroying atom instances and selector caches, force-destroying ecosystems is not at all recommended (and not really tested). There shouldn't ever be a need to force-destroy an ecosystem. We may remove the ability.
:::
...
- Use `instance.destroy()` and `ecosystem.selectorCache.destroyCache()` to manually destroy and force-destroy atom instances and selector caches.
- Use `instance.addDependent()` and `ecosystem.selectorCache.addDependent()` to manually add graph edges.
...
Now that you know how to control the lifecycle of atom instances, you're ready to learn the cool stuff. Let's start with learning how to [provide atom instances over React context](react-context).
