...
<LiveEditor ecosystemId="context/simple" resultVar="Parent">{`
...
Local atoms also always set [`ttl`](../api/classes/Atom#ttl) to `0`, which will clean up atom instances immediately when all components using the atom instance unmount.
