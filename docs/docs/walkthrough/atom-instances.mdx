...
Up till now, `useAtomState` has been our only way to interact with atoms. It's time to learn a new hook.
...
Atom instances have some more crazy features such as exporting functions and kicking of React suspense. To unlock these powers, you need [Atom APIs](atom-apis).
