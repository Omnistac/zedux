...
The one capability that `createInjector` gives you that nothing else does is access to the partially-created atom instance (along with its atom definition) currently being created.

This is hidden from everything else because on initial evaluation, the atom instance doesn't have many things you'd expect to exist on an instance - namely the 3 "meta data types" (store, exports, promise). These haven't been created yet. That's the purpose of the initial evaluation!
