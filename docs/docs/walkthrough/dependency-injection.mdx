...
Here we used [ion's `get`](../api/types/GetterUtils#get) to dynamically inject dependencies. We can also use any number of injectors, such as [`injectAtomValue()`](../api/injectors/injectAtomValue) or [`injectAtomInstance()`](../api/injectors/injectAtomInstance).
