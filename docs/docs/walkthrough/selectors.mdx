...
- How to manipulate the selector cache
...
We just learned about this Atom Getter in the [Atom Getters walkthrough](atom-getters). Anywhere you have access to Atom Getters, you can use the `select` Atom Getter to run an Atom Selector:
...
:::tip
Remember, when Atom Getters are called synchronously during Atom Selector or atom instance evaluation (like we did here), they register graph dependencies. This means our `userSettingsAtom` will reevaluate every time `getUserById`'s result changes!
:::
...
Sometimes you need a more complex, expensive selector that must not run unless it absolutely needs to - imagine sorting, filter, and mapping a big list that changes often. Atom Selectors _can_ be okay for complex selection logic sometimes. But usually you'll want more control.
...
The state factory of an ion receives an [Atom Getters object](atom-getters) as its first parameter. Any parameters after that are the actual params of the ion:
...
This Atom Getters parameter is basically the entire difference between atoms and ions.
...
Since you have full control over atom evaluations, prefer ions for the heavy lifting.
...
function Graph() {
  const joe = useAtomSelector(getUserById, 1)
  const jill = useAtomSelector(getUserById, 2)
...
Click `Log > Graph` in the above sandbox, open your browser console, and expand the `Flat` graph view. You'll see that `getUserById` has 2 entries in the graph - one for each set of params we passed it.
...
Every Atom Selector + params combo that creates a graph node also creates a "selector cache instance" that stores the last result of the selector. Every ecosystem has a `.selectorCache` that keeps track of these cache instances. Every cache instance includes these fields:

```ts
const { cacheKey, result } = cache
```

Where `cacheKey` is the string the cache is keyed by internally and `result` is the cached result. See [the SelectorCacheInstance class](../api/classes/SelectorCacheInstance) for more info.

The overarching Selector Cache has several methods that allow you to cache new selectors and find, invalidate, and destroy selector cache instances.

### `getCache`

Gets an Atom Selector cache. If the passed selector + params combo hasn't been cached before, `getCache` runs the selector, caches the result, and returns the new cache.

```ts
const cache = ecosystem.selectorCache.getCache(getUserById, [userId])
```

:::note
The 2nd argument to most Selector Cache methods is an array of the selector's params.
:::

### `weakGetCache`

Just like [the Weak Getters](atom-getters#weak-getters) for atom instances, the Selector Cache has a Weak Getter for selector caches. Returns the cache object **only if it exists**. Otherwise returns undefined.

```ts
const maybeCache = ecosystem.selectorCache.weakGetCache(getUserById, [userId])
```

:::note
This is different from `ecosystem.select()`. `weakGetCache` never runs the selector - it just returns the cache object if it exists. `ecosystem.select()` returns the cached _result_ if a cache exists. Otherwise it runs the selector statically (without registering graph dependencies) and returns that result.
:::

### `invalidateCache`

Forces Zedux to reevaluate a cached Atom Selector.

```ts
ecosystem.selectorCache.invalidateCache(getUserById, [userId])
```

### `destroyCache`

Destroys an Atom Selector cache (if it exists).

```ts
ecosystem.selectorCache.destroyCache(getUserById, [userId])
```
...
Atom Selectors are a decent choice - they are "globally" cached in the ecosystem as long as the selector reference is stable. But they _can_ evaluate unnecessarily in React sometimes. In some cases, we can circumvent that by pre-caching the selector e.g. in the ecosystem's `onReady` function:

```ts
const rootEcosystem = createEcosystem({
  id: 'root',
  onReady: ecosystem => {
    ecosystem.selectorCache.getCache(threadHogger)
  },
})
```
...
Using [`injectMemo`](../api/injectors/injectMemo) in an ion gives us maximum selection and caching powers here:
...
You may have noticed in the above very specific example that the `injectMemo` is redundant. Since `expensiveAtom` has only one dependency, it will only reevaluate when `someAtom`'s value changes. And since we pass that value straight to `injectMemo`'s deps array, we're effectively running `threadHogger` every single time this atom evaluates anyway. Let's do away with it:
...
The main reason AtomSelectors were added to Zedux is to provide an easy way to dynamically add and remove graph edges in React components. We already have this capability in atoms thanks to injectors and Atom Getters:
...
Here we declared an Atom Selector inline. The function gets recreated every time `MyComponent` renders. Since the reference changes, Zedux thinks the new Atom Selector is different. **Zedux will run this Atom Selector every time `MyComponent` renders.**
...
Sometimes this works beautifully, but in this case, our selector needs access to the `shouldUse1` state. We therefore _do_ want the Atom Selector to re-run sometimes, but only when `shouldUse1` changes.
...
Now our Atom Selector will only run exactly as often as it needs to. But there is another way:

### Passing Arguments

AtomSelectors take the AtomGetters object as their first argument, but they can also take any number of additional arguments. These can be passed as the rest params of `useAtomSelector`, `injectAtomSelector`, `ecosystem.select`, and other `select` AtomGetters.
...
We can also use special [AtomSelectorConfig](../api/types/AtomSelector#atomselectorconfig) objects to control when Zedux reruns an Atom Selector and whether the new result has "changed" and should cause a rerender.
...
```ts
// this selector will only rerun when the `filters.name` we pass it changes.
