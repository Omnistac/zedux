...
Where `cacheKey` is the string the cache is keyed by internally and `result` is the cached result. See [the SelectorCache class](../api/classes/SelectorCache) for more info.
...
:::note
The 2nd argument to most Selectors methods is an array of the selector's params.
:::
