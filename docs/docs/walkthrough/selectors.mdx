...
function Child() {
  ...
  const currentUserName = useAtomSelector(
    isInline
      ? {
          resultsComparator: () => true,
          selector: ({ select }: AtomGetters) => {
            return select(getCurrentUserName)
          },
        }
      : getCurrentUserName,
    text,
  ...
  return (
    <div>
      <button onClick={() => setIsInline(state => !state)}>toggle</button>
      <input
        onChange={event => setText(event.currentTarget.value)}
        value={text}
      />
      <span>Hello, {currentUserName}</span>
      <button onClick={() => idInstance.setState(1)}>One</button>
      <button onClick={() => idInstance.setState(2)}>Two</button>
      <button onClick={() => idInstance.setState(3)}>Three</button>
    </div>
  )
}
