...
Where `cacheKey` is the string the cache is keyed by internally and `result` is the cached result. See [the SelectorCacheItem class](../api/classes/SelectorCacheItem) for more info.
