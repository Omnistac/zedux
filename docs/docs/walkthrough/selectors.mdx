...
const { name } = useAtomSelector(
  {
    shouldRun: ([newId], [oldId]) => newId !== oldId,
    shouldTriggerUpdate: (newUser, oldUser) => newUser.name !== oldUser.name,
...
<LiveEditor ecosystemId="selectors/composition" resultVar="Username">{`
const usersAtom = atom('users', [
  { id: 1, name: 'Joe' },
  { id: 2, name: 'Jill' },
  { id: 3, name: 'Jim' },
