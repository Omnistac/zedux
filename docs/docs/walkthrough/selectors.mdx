...
Just like [the Weak Getters](atom-getters#weak-getters) for atom instances, the Selector Cache has Weak Getters for selector caches. `find` returns the cache object **only if it exists**. Otherwise returns undefined.

```ts
const maybeCache = ecosystem.selectorCache.find(getUserById, [userId])
```

:::note
This is different from `ecosystem.select()`. `selectorCache.find()` never runs the selector - it just returns the cache object if it exists. `ecosystem.select()` returns the cached _result_ if a cache exists. Otherwise it runs the selector statically (without registering graph dependencies) and returns that result.
:::
