...
For TS users, `.send()` only accepts strings that were specified as signal names during machine creation. In this example, passing anything but the string `'timer'` gives a TS error:
