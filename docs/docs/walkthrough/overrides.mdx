...
Zedux' Dependency Injection (DI) model is extremely dynamic. This walkthrough has taught several ways to inject atoms and create the DI graph so far. But Zedux has another DI superpower: Overrides.
...
:::note
Atom overrides created manually like this will log errors unless you turn off deduping by passing [`dedupe: false`](../api/classes/Ecosystem#dedupe) when creating the ecosystem.
:::

The nice thing about `.override()` for TS users is that it will tell you if the override doesn't match the overridden atom's state, params, exports, or promise type.
