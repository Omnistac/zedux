...
If all else fails, you can pass [`internalTypes.ignore`](../api/constants/internalTypes#ignore) as an action's `meta` property to prevent Zedux from reevaluating this atom on this particular state change.
