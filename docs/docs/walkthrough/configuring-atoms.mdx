...
We've learned how to use [Atom APIs](atom-apis) to configure an atom instance's promise and exports. But there are a few more configuration options we haven't covered.
...
Here we created an atom that uses the `window` object without checking that it exists. This atom would throw an error if it was run, e.g., in a Node environment, so we gave it a flag to indicate this - `'browser-only'`.

:::tip
The flags can be any strings; you decide what they are.
:::

One purpose of flags is to tell Zedux to log a warning when an atom is used in an unsafe environment. To tell Zedux which flags are safe, use the `flags` option on your ecosystem:
...
Here we told Zedux that atoms with the flag `'browser-only'` are safe in this ecosystem. That includes our `scrollListenerAtom` from above.

Now say we want to run our code in Node (e.g. for SSR). We can tell Zedux that atoms with the flag `'browser-only'` are not safe by _not_ including it in the ecosystem's list of flags:
...
Now if we try to use `scrollListenerAtom` in this ecosystem, Zedux will log a warning.

```tsx live ecosystemId=configuring-atoms/flags resultVar=Parent
const scrollListenerAtom = atom(
  'scrollListener',
  () => {
...
To turn off flag warnings, just don't give your ecosystem a `flags` prop.

```ts
ecosystem({ flags: [] }) // warns on every flag
...
```tsx live ecosystemId=configuring-atoms/defaultTtl resultVar=Parent
...
We've learned that ecosystems have a `defaultTtl`, atoms have a `ttl` config option, and Atom APIs have a `setTtl` function. If you set ttl in multiple places, the "most specific" will take priority. Here's a little visual:

![ecosystems -> atoms -> atom instances](/img/diagrams/config-specificity.png)

Ecosystem-level config is the most general, atom-level config is more specific, and atom instance-level config is the most specific. Thus `api().setTtl()` takes priority over everything and `ttl` takes priority over `defaultTtl`.
