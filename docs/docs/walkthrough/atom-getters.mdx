...
const exampleAtom = atom('example', () => {
  ...
  ecosystem.inspectGraph() // the dep won't be there (it doesn't exist yet!)
})
...
const exampleAtom = atom('example', () => {
  ...
  injectEffect(() => {
    ecosystem.inspectGraph() // effects are deferred; the dep exists now!
  }, [])
})
```
...
Returns an atom instance's value **if the atom instance exists**. Otherwise returns undefined.

```ts
const val = ecosystem.weakGet(myAtom)
const paramsExample = ecosystem.weakGet(myAtom, ['param 1', 'param 2'])
```

### `weakGetInstance`

Returns an atom instance **if the atom instance already exists**. Otherwise returns undefined.

```ts
const instance = ecosystem.weakGetInstance(myAtom)
const paramsExample = ecosystem.weakGetInstance(myAtom, ['param 1', 'param 2'])
```

:::note
There is no `weakSelect` - `ecosystem.select` never caches anything, but it will return the cached selector result if the passed Atom Selector has been cached before.
:::
...
- The Weak Getters (`weakGet` and `weakGetInstance`) return undefined if no atom instance exists yet rather than automatically creating it.
