...
When a string is passed, `ecosystem.find()` returns the first atom instance it finds whose keyHash contains the passed string (case-insensitive).

### `findAll`

Returns an object containing all instances of the passed atom **if any atom instances exist**, keyed by each instance's keyHash. Otherwise returns an empty object.
...
When a string is passed, `ecosystem.findAll()` returns all atom instance whose keyHash contains the passed string (case-insensitive).
