...
In the quick start, we learned that injectors are like hooks for atoms. But some operations don't fit well into the injector paradigm. Injectors, like hooks, should be composable and usable multiple times throughout an atom state factory and other injectors.
...
To create an Atom API, we simply wrap state in a call to `api()` before we return it from a state factory:
...
Atoms can export anything via an Atom API:
...
We learned in the [atom instances walkthrough](atom-instances) that atom instances have a `.exports` property. Here we're seeing it in action. Atom APIs have a `.setExports()` method that allows us to define an atom instance's exports the first time it evaluates.

Exports must be an object and can contain anything. As you can imagine, exports are another big feature that sets Zedux apart from other atom libraries.
...
This is poor man's async handling (there are several bugs in the above example). Let's upgrade to Suspense.

## Suspense

Atoms can be used as suspending resources in React. To suspend, we simply pass a promise to a returned Atom API's `.setPromise()` method:
...
In Zedux, atoms always evaluate synchronously. This allows Zedux' DI to be as dynamic as possible and simplifies a lot for you. It also means that state factories can't be asynchronous functions.
...
The object returned from `api()` is an instance of [the `AtomApi` class](../api/classes/AtomApi). It has a few **readonly** properties that reference the values we set on it:
...
Atom APIs are a convenient way to pass stores + exports around. You may find yourself working with multiple Atom APIs when using custom injectors. Atom APIs have a `.addExports()` method specifically for merging exports together from multiple sources:
...
If these custom injectors return Atom APIs with stores, you can also use store composition to create a new Atom API from the injected APIs:
...
Unlike Recoil, Zedux doesn't provide any promise utilities like `waitForAll`/`waitForNone`. This is because Zedux relinquishes all control over async flows to you, the programmer. This means you can use native Promise APIs or any other async management layer (yes, even RxJS) to control serialization and parallelization of async flows.
...
:::tip
Click `Reset` in the above sandbox and watch
:::

## Recap

- Create an Atom API with `api(stateOrStore)`. Return an Atom API from a state factory to make it **the** API of an atom instance.
- Use `.setExports()` to export an object containing anything and set that object as the instance's `.exports` property.
...
We know a lot about atoms themselves at this point. It's time to zoom out and look at the [bigger picture](ecosystems).
