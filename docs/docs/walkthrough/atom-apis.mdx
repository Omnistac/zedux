...
You can create an Atom API anywhere, but only the Atom API returned from the state factory will become _the_ atom instance's API.
...
Atom instances have a `.exports` property. Atom APIs have a `.setExports()` method that defines an atom instance's exports (when the atom API is returned from the state factory). These exports are set on initial evaluation and ignored on evaluations after that.
