...
This config object has many useful properties. We'll look at some of these now. See [`AtomConfig`](../api/types/AtomConfig) for the full rundown.
...
`ttl` can be set to `0` to clean up stale atom instances immediately.

### Flags

An atom can be given any number of string "flags" that serve various purposes, such as logging warnings when certain atoms are run in "unsafe" environments - for example, to indicate that an atom that sends network requests should be overridden in tests, or that an atom that uses browser APIs shouldn't run in Node. Flags can also be used during SSR dehydration/hydration to mark atoms whose state can't be serialized.

## Stores

Every instance of every atom creates a light-weight [Zedux store](../api/classes/Store). Stores manage state and serve as a message bus for running side effects based on dispatched actions or state changes. Most stores in Zedux use the "zero-config" approach - no reducer hierarchy, just simple getting and setting.
...
Injectors are inspired by React hooks. Using them should feel similar to using hooks. Like hooks, injectors should only be used at the top level of state factory functions - outside any if statements or loops and before any early returns.

We'll look at some of the most common injectors here.

### `injectStore`

Creates a store. The returned store is a stable reference; like a React ref, it will not change on subsequent evaluations. This means you can then return the store from the state factory.

The first parameter can be either the initial state of the store or a function that returns a custom store.
...
Notice that we returned the store from the state factory. This is important! Zedux automatically creates a store for every atom instance **unless** you return a store from the state factory function. This essentially tells Zedux, "Don't create a store for me! I created my own. Use this one."
...
`injectStore`'s second parameter is a config object. It can currently take one property - `subscribe` - a boolean indicating whether this atom instance should subscribe to this store. Default is `true`, meaning state updates on the injected store will trigger a reevaluation of the current atom instance.

```ts
// every time this store's state changes, this state factory will rerun:
...
The injector equivalent of React's [`useEffect`](https://reactjs.org/docs/hooks-reference.html#useeffect). Runs a deferred side effect every time its dependencies change.

```ts
const myEffectAtom = atom('myEffect', () => {
...
When used synchronously during atom evaluation (like we did here), `get`, `getInstance`, and `select` update the atom instance's dependency graph dynamically.

When used asynchronously, AtomGetters will not update the dependency graph; they simply return the value. (We'll cover ways to asynchronously update an atom instance's dependency graph [later](/not-done))

## Atom Api

Some operations don't fit well into the injector paradigm. Injectors, like hooks, should be composable and usable multiple times throughout an atom state factory and other injectors. But atoms have a few one-off operations - operations that should only be performed once. For these situations, we use an AtomApi.
...
AtomApis have more features that we'll cover eventually. Or check out the [AtomApi reference](../api/classes/AtomApi) for all the power.
...
:::tip When should I use an ion instead of an atom?

You can make your own rules, but here's why ions exist:

Ions are like AtomSelectors (which we'll cover in the [selectors walkthrough](selectors)) in atom form - we recommend using them when you want a selector, but need more control over the memoization details or need to run side effects.
:::

## Recap

- `atom()` can take a config object.
- Stale atoms can be destroyed automatically using `ttl`.
...
- Atoms can be turned into simple React Query-esque "queries" by returning `api(promise)`.
- Ions are special atom/AtomSelector hybrids.
