...
To share your ecosystem setup across realms, you could share each ecosystem and manually set each Zedux instance's `internalStore`'s state yourself. But the more complete approach is to share Zedux' internalStore across the realms and use `setInternalStore` to make Zedux reuse the exact same `internalStore` in every realm.
