...
store.actionStream().subscribe({
  ...
  <Item name="dispatch">
    ...
    <p>
      ...
      (including action streams) will be notified of the action.
    </p>
    <p>
      ...
      Zedux' store composition model works and is very low-level.
    </p>
    <p>
...
store.dispatch({ type: internalTypes.hydrate, payload: 'myState' })`}</Ts>
  ...
  <Item name="setStateDeep">
    ...
    <p>
      IMPORTANT! Cannot be used to delete keys. Use <code>.setState()</code> for
      that. <code>.setStateDeep()</code> is only for recursively adding/updating
      keys.
    </p>
    <p>
      Generates and effectively dispatches a special{' '}
...
store.dispatch({
  ...
  <Item name="use">
    <p>
      The code-splitting wizard of the Zedux realm. <code>.use()</code> modifies
