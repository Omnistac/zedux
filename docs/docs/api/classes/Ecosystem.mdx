...
<LiveEditor ecosystemId="Ecosystem/modifying-overrides" resultVar="Swapper">{`
...
When `.reset()` is called, the previous context (if any) will be passed as the second parameter to [the `preload` function](../types/EcosystemConfig#preload) as part of the reset.
...
Optional. An object. The new context to set on this ecosystem. If not passed, the existing context will be reused. The ecosystem's `context` property will be set to this before `preload()` is called. The previous context will be passed to `preload()` as the second parameter.

```ts
const myEcosystem = createEcosystem({
  context: { someField: 'some val' },
  id: 'my',
  preload: (ecosystem, prevContext) =>
