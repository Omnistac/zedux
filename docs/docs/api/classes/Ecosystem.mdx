...
<Legend>
  <Item name="complexParams">
    <p>
      A boolean. This is set to the <code>complexParams</code> value you passed
      via <Link to="../types/EcosystemConfig">EcosystemConfig</Link> when
...
<Legend>
  <Item name="addOverrides">
    <p>
      ...
      implementations. All existing instances of atoms in the passed list will
      be force-destroyed, allowing their dependents to recreate them.
    </p>
    <p>Signature:</p>
    <Ts>{`addOverrides = (overrides) => void`}</Ts>
    <Legend>
      <Item name="overrides">
        Required. An array of <Link to="Atom">atoms</Link>.
      </Item>
    </Legend>
  </Item>
  <Item name="dehydrate">
  ...
  transform?
    ...
    <Legend>
      <Item name="exclude">
        <p>
          Optional. An array of atoms and/or case-insensitive fuzzy-search
          strings.
        </p>
        <p>
          Any atom instances of the passed atoms or whose keyHash matches any of
          the passed strings will be excluded from the snapshot.
        </p>
      </Item>
      <Item name="excludeFlags">
        <p>Optional. An array of strings.</p>
        <p>
          Any atom instance whose atom's <Link to="Atom#flags">flags</Link>{' '}
          contain any of the passed strings will be excluded from the snapshot.
        </p>
      </Item>
      <Item name="include">
        <p>
        ...
        <p>
          Only atom instances of the passed atoms or whose keyHashes match any
          of the passed strings will be included in the snapshot.
        </p>
        <p>
      ...
      <Item name="includeFlags">
        <p>Optional. An array of strings.</p>
        <p>
          Only atom instance whose atom's <Link to="Atom#flags">flags</Link>{' '}
          contain any of the passed strings will be included in the snapshot.
        </p>
        <p>
          Excludes take priority over includes - if an atom instance is excluded
          via <code>excludes</code> or <code>excludeFlags</code> <i>and</i>{' '}
      ...
      <Item name="Returns">
        <p>
          An object mapping atom instance keyHashes to the transformed form
          (unless <code>transform: false</code>) of their current state.
        </p>
      </Item>
    </Legend>
  </Item>
  <Item name="destroy">
    <p>
      ...
      from the{' '}
      <Link to="../utils/internal-store-utils#internalstore">
        internalStore
      </Link>
      .
    </p>
...
instance.setState('new state')`}</Ts>
  ...
  <Item name="hydrate">
    ...
    <Legend>
      <Item name="snapshot">
        <p>
          Required. An object. The keys of this object are keyHashes
          corresponding to atom instance that may or may not exist in the
          ecosystem yet.
        </p>
      </Item>
      <Item name="config">
  ...
  <Item name="inspectInstances">
    <p>Get all atom instances in the ecosystem.</p>
    <p>Signature:</p>
    <Ts>{`inspectInstances = (atom?) => instances`}</Ts>
    <Legend>
      <Item name="atom">
        An atom or fuzzy search string. If an atom is passed,{' '}
        <code>.inspectInstances()</code> returns only instances of the passed
        atom. If a string is passed, <code>.inspectInstances()</code> returns
        only instances whose keyHash includes the passed string
        (case-insensitive).
      </Item>
      <Item name="Returns">
        An object of all Active and Stale atom instances in the ecosystem, keyed
        by the instance's keyHash.
      </Item>
    </Legend>
  </Item>
  ...
  <Item name="removeOverrides">
    <p>
      ...
      <code>.setOverrides()</code>. All existing instances of atoms in the
      passed list will be force-destroyed, allowing their dependents to recreate
      them using the original, non-overridden atom.
    </p>
    <p>Signature:</p>
    <Ts>{`removeOverrides = (overrides) => void`}</Ts>
    <Legend>
      <Item name="overrides">
        Required. An array of <Link to="Atom">atoms</Link>. If any haven't been
        set as overrides previously, they'll be ignored.
      </Item>
    </Legend>
  </Item>
  <Item name="reset">
...
// old context: { someField: 'some val' } new context: { someField: 'new val' }`}</Ts>
  ...
  <Item name="select">
    <p>
      ...
      hasn't been cached yet, runs the selector but does _not_ cache the result.
      Use{' '}
      <Link to="SelectorCache">
        <code>ecosystem.selectorCache.getCache()</code>
      </Link>{' '}
...
const withArgs = myEcosystem.select(myAtomSelector, 'param 1', 'param 2')`}</Ts>
  ...
  <Item name="setOverrides">
    <p>
      ...
      overrides. All instances of atoms in either the new or old lists will be
      force-destroyed, allowing their dependencies to recreate them.
    </p>
    <p>
    ...
    <p>
      Accepts an array of atoms. This will be set as the new{' '}
      <Link to="#overrides">
        <code>.overrides</code>
      </Link>{' '}
      property.
  ...
  <Item name="weakGet">
    <p>
      Works exactly like{' '}
      <Link to="#get">
        <code>ecosystem.get()</code>
      </Link>{' '}
      except it does NOT create the atom if it doesn't exist. In that case,{' '}
      <code>.weakGet()</code> returns undefined
    </p>
    <Ts>{`val = ecosystem.get(myAtom, [...params]) // creates the atom instance if needed
val = ecosystem.weakGet(myAtom, [...params]) // never creates an atom instance`}</Ts>
  </Item>
  <Item name="weakGetInstance">
    <p>
      Works exactly like{' '}
      <Link to="#getinstance">
      ...
      <code>.weakGetInstance()</code> returns undefined
    </p>
    <Ts>{`val = ecosystem.getInstance(myAtom, [...params]) // creates the atom instance if needed
val = ecosystem.weakGetInstance(myAtom, [...params]) // never creates an atom instance`}</Ts>
  </Item>
  <Item name="why">
    <p>
      Returns a list of{' '}
