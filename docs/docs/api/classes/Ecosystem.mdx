...
<Legend>
  ...
  <Item name="dedupe">
    <p>
      A boolean. This is set to the <code>dedupe</code> value you passed via{' '}
      <Link to="../types/EcosystemConfig#dedupe">EcosystemConfig</Link> when
      creating this ecosystem. If not set, defaults to <code>true</code>.
    </p>
    <p>
      Whether to log an error if the ecosystem encounters multiple atom
      templates with the same key.
    </p>
    <p>
      This can be useful to ensure atom keys are unique. Only atom templates
      created via{' '}
      <Link to="Atom#override">
        <code>atom.override()</code>
      </Link>{' '}
      will be ignored. Any other duplicates will log an error.
    </p>
    <p>
      Turn this off if you need to support multiple windows, since each window
      creates its own atom template references.
    </p>
  </Item>
  ...
  <Item name="flags">
    ...
    <p>
      These work in conjunction with <Link to="Atom#flags">atom flags</Link> to
      raise warnings when unsafe atoms are not overridden in certain
      environments.
    </p>
    <p>
...
<Legend>
  ...
  transform?
    ...
    <Legend>
      <Item name="exclude">
        ...
        <p>
          Any atom instances of the passed atoms or whose id matches any of the
          passed strings will be excluded from the snapshot.
        </p>
      </Item>
      ...
      <Item name="include">
        ...
        <p>
          Only atom instances of the passed atoms or whose ids match any of the
          passed strings will be included in the snapshot.
        </p>
        <p>
  ...
  <Item name="find">
    <p>
      ...
      <code>.find()</code> returns undefined
    </p>
    <Ts>{`val = ecosystem.getInstance(myAtom, [...params]) // creates the atom instance if needed
val = ecosystem.find(myAtom, [...params]) // never creates an atom instance`}</Ts>
    <p>
  ...
  <Item name="findAll">
    ...
    <Legend>
      <Item name="atom">
        ...
        <code>.findAll()</code> returns only instances of the passed atom. If a
        string is passed, <code>.findAll()</code> returns only instances whose
        id includes the passed string (case-insensitive).
      </Item>
      <Item name="Returns">
