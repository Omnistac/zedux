...
There are many aspects of an atom instance's behavior you can overwrite when extending this class. This is an extremely advanced feature. We're not documenting it yet as the internals of this class may change.
...
<Legend>
  ...
  <Item name="atom">
    <p>
      A reference to the <Link to="Atom">atom</Link> that this atom instance was
      created from.
    </p>
  </Item>
  <Item name="ecosystem">
...
const importAtom = atom('import', () => {
  ...
  <Item name="keyHash">
    <p>
      A string representing the stable, unique key of this atom instance in the
      ecosystem. This keyHash is the combination of the{' '}
      <Link to="Atom#key">atom's key</Link> and a stable stringification of the{' '}
      <Link to="#params">
        <code>params</code>
      </Link>{' '}
    ...
    <Ts>{`ecosystem.getInstance(atom('a', null)).keyHash // 'a'
...
instanceB.params // ['a', 'b']`}</Ts>
    ...
    <Ts>{`// These params are the "same" in Zedux' eyes:
...
function Shout() {
  ...
  <Item name="promise">
    ...
    <p>
      ...
      reference will not change on subsequent evaluations.
    </p>
  </Item>
  <Item name="store">
    <p>
...
function Coin() {
  ...
  return <button onClick={invalidate}>{isHeads ? 'Heads' : 'Tails'}</button>
    ...
    <p>
      A reference to this functino is returned from{' '}
