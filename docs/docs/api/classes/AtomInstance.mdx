...
All standard "atom instances" are actually instances of this class. When Zedux instantiates [Ions](Ion) and [Standard Atoms](Atom), it's just creating instances of this class.
...
A string representing the status of the atom instance. Can be `Active`, `Destroyed`, `Stale`, or `Initializing`.

You typically shouldn't need to look at this, but there are some cases where you might. For example, when holding a reference to an atom instance outside of React that you expect to be destroyed, you may need to check that `instance.activeState !== 'Destroyed'` before you use it (and if it is, update your local reference using [`ecosystem.getInstance()`](Ecosystem#getinstance)).
...
The only exception to the serializable rule is other atom instances. That's right! You can pass an atom instance to another atom instance. You can then use [`get`](../types/AtomGetters#get), [`injectAtomValue`](../injectors/injectAtomValue), or any other dynamic injector to register a [dynamic graph dependency](../glossary#dynamic-graph-dependency) on the passed atom instance.

```tsx live ecosystemId=AtomInstance/shout resultVar=Shout
...
Optional. Default: `false`. If passed, Zedux will not prevent destruction if this atom instance has dependents.

### `.dispatch`

A bound function property. An alias for `.store.dispatch()`.

```ts
const instance = useAtomInstance(myAtom)
const newState = instance.dispatch(myActor()) // same as instance.store.dispatch

// since this function is bound, dereferencing is fine:
const { dispatch } = instance
```

To prevent this atom instance from reevaluating, set [`internalTypes.ignore`](../constants/internalTypes#ignore) as the `meta` property of the dispatched action:

```ts
import { internalTypes } from '@zedux/react'

instance.dispatch({ type: 'action-type', meta: internalTypes.ignore })
```
...
```tsx live ecosystemId=AtomInstance/invalidate resultVar=Coin
...
A bound function property. An alias for [`.store.setState()`](Store#setstate).
...
To prevent this atom instance from reevaluating, pass [`internalTypes.ignore`](../constants/internalTypes#ignore) as the `meta` property:

```ts
import { internalTypes } from '@zedux/react'

instance.setState(newState, internalTypes.ignore)
```

### `.setStateDeep`

A bound function property. An alias for [`.store.setStateDeep()`](Store#setstatedeep).

```ts
const instance = useAtomInstance(myAtom)
const newState = instance.setStateDeep('new state')

// since this function is bound, dereferencing is fine:
const { setStateDeep } = instance
```
...
instance.setStateDeep(newState, internalTypes.ignore)
```
