...
An ion getter is really just an evaluator that receives a special [GetterUtils object](../types/GetterUtils) as its first parameter.

An ion setter is similar to a [SetStateInterceptor](../types/SetStateInterceptor). But it receives a special [SetterUtils object](../types/SetterUtils) as its first parameter and the value passed to [`instance.setState()`](AtomInstance#setstate) as its second parameter. Unlike SetStateInterceptors, ion setters do not need to return the new state.
