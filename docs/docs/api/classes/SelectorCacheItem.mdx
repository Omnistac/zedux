...
Every time an [ecosystem](Ecosystem)'s [`.selectorCache`](SelectorCache) caches an atom selector, the "cached selector" is really just an instance of this class. Thus, an instance of this class is what's returned from various SelectorCache methods like [`.getCache()`](SelectorCache#getcache) and [`.inspectCacheItems()`](SelectorCache#inspectcacheitems).
...
const cacheItem = ecosystem.selectorCache.getCache(getOddNumbers)
```
...
<Legend>
  ...
  <Item name="cacheKey">
    <p>
      A string. The key Zedux uses to refer to this SelectorCacheItem in the
      SelectorCache and the ecosystem graph.
    </p>
    <p>
  ...
  <Item name="isDestroyed">
    <p>
      ...
      like [atom instances](AtomInstance#activestate). This is all they get.
    </p>
    <p>
      ...
      SelectorCacheItems you're using outside React/atoms.
    </p>
  </Item>
