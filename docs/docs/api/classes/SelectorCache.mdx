...
<Legend>
  ...
  <Item name="find">
    <p>
      ...
      for the given selector + args combo. However, unlike <code>
        .getCache()
      </code>, <code>.find()</code> does not run the selector or cache the result
      if it hasn't been cached already - it simply returns undefined.
    </p>
    <p>
      ...
      when passed, returns the first found SelectorCacheItem whose cacheKey includes
      ...
      <Item name="selectableOrName">
        ...
        <ul>
          ...
          <li>
            ...
            (accepted for consistency with similar methods, but really you'd
            never pass this)
          </li>
        </ul>
        <p>
  ...
  <Item name="getCache">
    <p>
      ...
      never been cached before, <code>.getCache()</code> runs the selector and
      caches the result before returning the cache item.
    </p>
    <p>
      Contrast this to{' '}
