...
<Tabs>
  ...
  <A extends AtomBase<any, [...any], any>>(atom: A, params: AtomParamsType<A>, config?: ZeduxHookConfig): AtomInstanceType<A>;
  <AI extends AtomInstance<any, [...any], any, any>>(instance: AI, params?: [], config?: ZeduxHookConfig): AI;
}`)}
</Tabs>
...
<InjectAtomInstanceLegend
  ...
  thirdParam={
    <Item name="config">
      ...
      <Ts>{`{ operation, suspend }`}</Ts>
      <Legend>
        <Item name="operation">
          <p>
            Optional. A string. Default <code>'useAtomInstance'</code>
          </p>
          <p>
            You usually won't need to worry about this. Passing an operation
            string can help with debugging e.g. if the instance throws an error
            or when inspecting the instance's list of injectors.
          </p>
        </Item>
        <Item name="suspend">
          <p>
          ...
          <p>
            Pass <code>false</code> to prevent React from suspending until the
            resolved instance's{' '}
            <Link to="../classes/AtomInstance#promise">promise</Link> is
            resolved.
          </p>
        </Item>
      </Legend>
