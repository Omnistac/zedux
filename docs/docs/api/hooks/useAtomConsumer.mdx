...
<Tabs>
  ...
  <A extends AnyAtom>(atom: A): AtomInstanceType<A> | Record<string, never>;
  <A extends AnyAtom>(atom: A, defaultParams: AtomParamsType<A>): AtomInstanceType<A>;
  <A extends AnyAtom>(atom: A, throwIfNotProvided: true): AtomInstanceType<A>;
}`)}
</Tabs>
