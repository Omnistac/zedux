...
Caches the selector's result in the ecosystem's [`.selectorCache`](../classes/SelectorCache) if it hasn't been cached yet. If this selector has been cached before with the exact same arguments, the selector doesn't run, and `useAtomSelector()` returns the cached result.
...
<Tabs>
  ...
  {tab2(
    `declare const useAtomSelector: <T, Args extends any[]>(selectable: Selectable<T, Args>, ...args: Args) => T`
  )}
</Tabs>
