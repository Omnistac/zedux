...
Accepts a promise factory function, a list of dependencies (like `injectMemo` and `injectEffect`), and an optional config object.
...
Use this injector to obtain a stable promise reference that can be passed to [`AtomApi#setPromise()`](../classes/AtomApi#setpromise) to trigger React suspense.
