...
Caches the selector's result in the ecosystem's [`.selectorCache`](../classes/SelectorCache) if it hasn't been cached yet. If this selector has been cached before with the exact same arguments, the selector doesn't run, and `injectAtomSelector()` returns the cached result.
...
<Tabs>
  ...
  {tab2(
    `declare const injectAtomSelector: <T, Args extends any[]>(selectable: Selectable<T, Args>, ...args: Args) => T`
  )}
</Tabs>
  ...
  <Legend>
    <Item name="selectable">
      <p>
        ...
        or a <Link to="../classes/SelectorCacheItem">SelectorCacheItem</Link>{' '}
        instance.
      </p>
      <p>
      ...
      <p>
        If an SelectorCacheItem is passed, <code>{name}()</code> simply returns
        the cached value.
      </p>
      <p>
    ...
    <Item name="args">
      ...
      <p>
        ...
        <Link to="../types/EcosystemConfig#complexselectorparams">
          <code>complexSelectorParams</code>
        </Link>{' '}
