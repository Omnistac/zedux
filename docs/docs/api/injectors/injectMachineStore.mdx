...
store.getContext() // { foo: 'bar' }`}</Ts>
  ...
  <Item name="config">
    ...
    <Legend>
      <Item name="guard">
        <p>
          Optional. A function that receives the MachineStore's current{' '}
          <code>.context</code> value and should return a boolean.
        </p>
        <Tabs>
          {tab1(`guard = (context) => boolean`)}
          {tab2(`guard?: (
  ...
  nextState: StateNames
        ...
        <p>
          This function is called every time the state receives a signal that
  ...
  <Item name="Returns">
    ...
    <p>
      If returning this store from a state factory, you may also want to export
      some of the MachineStore's properties like <code>.send</code>,{' '}
      <code>.is</code>, and <code>.setContext</code>
    </p>
  </Item>
</Legend>
