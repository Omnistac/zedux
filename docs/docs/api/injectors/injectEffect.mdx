...
This is _almost_ exactly equivalent to React's [`useEffect()`](https://reactjs.org/docs/hooks-reference.html#useeffect) hook. The only difference being that `injectEffect()` accepts async functions (a function that returns a promise).
...
This injector does not run the callback when the [`ssr` ecosystem config option](../types/EcosystemConfig#ssr) is set to true.
...
<Tabs>
  {tab1(`injectEffect = (effect, deps?) => void`)}
  {tab2(
    `declare const injectEffect: (effect: EffectCallback, deps?: InjectorDeps) => void`
  )}
</Tabs>
...
<Legend>
  <Item name="effect">
    ...
    <Tabs>
      {tab1(`effect = () => cleanup`)}
      {tab2(`effect: () => (() => void) | void | Promise<any>`)}
    </Tabs>
    <Legend>
      <Item name="Returns">
