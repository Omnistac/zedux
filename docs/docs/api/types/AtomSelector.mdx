...
const stateSnapshot = useAtomSelector({
  // shouldRun isn't called on the first run
  shouldRun: () => false,
  selector: mySelector,
})
...
const limitedRuns = useAtomSelector(
  {
    shouldRun: (newArgs, oldArgs) => !_.isEqual(newArgs, oldArgs),
    selector: mySelector,
  },
  arg1,
  arg2
...
Everywhere Zedux accepts an AtomSelector, it also accepts an AtomSelectorConfig object. This object has a required `selector` field and optional `shouldRun` and `shouldTriggerUpdate` fields.

- `selector` - The AtomSelector whose args and result we're configuring.

- `shouldRun` - A function that accepts two parameters - the new args and old args - and should return a boolean. If this function returns false, Zedux will not run the AtomSelector again. This function is not called on the first run.

- `shouldTriggerUpdate` - A function that accepts two parameters - the new result and old result - and should return a boolean. If this function returns false, Zedux will not trigger a component rerender (when used from `useAtomSelector`) or an atom reevaluation (when used from `injectAtomSelector` or ion's `select`). This function is not called after the first run.
...
function CountBy5() {
  ...
  const counter = useAtomSelector({
    shouldTriggerUpdate: newCount => !(newCount % 5),
