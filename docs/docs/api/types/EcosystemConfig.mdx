...
interface EcosystemConfig<Context extends Record<string, any> = any> {
  ...
  id?: string
  overrides?: Atom[]
  preload?: (ecosystem: Ecosystem, prevContext?: Context) => void
}
```
...
Receives the freshly initialized or reset ecosystem. On reset also receives the previous context object as the second parameter.
