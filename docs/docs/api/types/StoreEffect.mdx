...
<Legend>
  <Item name="action">
    ...
    <p>
      ...
      <Link to="../classes/Store#setstate">store.setState()</Link> and similar
      APIs always generate "psuedo actions" representing the change.
    </p>
  </Item>
  <Item name="error">
    <p>
      If the dispatch or setState call errored, this will be a reference to the
  ...
  <Item name="newState">
    ...
    <p>
      Note that effects subscribers run after the state has been completely
      updated, so this value will be exactly equal to{' '}
