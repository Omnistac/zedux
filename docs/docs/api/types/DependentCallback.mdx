...
A low-level function used when manually graphing e.g. with [`ecosystem.selectorCache.addDependent()`](../classes/SelectorCache#adddependent) or [`atomInstance.addDependent()`](../classes/AtomInstance#adddependent). This function is called with special "GraphEdgeSignals" when Zedux needs to push some new information about the dependency to the dependent.
