...
:::tip
AtomSelectors sometimes have config that's inherent to how they operate. Rather than exporting the selector function and requiring consumers to specify the config, export the config object directly (tightly coupling the selector to its mandatory config):
:::
