...
interface EvaluationReason<State = any> {
  ...
  sourceKey?: string // e.g. an atom key like "todos" or "userList"
...
<Legend>
  ...
  <Item name="sourceKey">
    <p>
      If the source dependency was another atom instance, this will be the key
      of that atom instance's atom. Otherwise, this will be undefined.
    </p>
  </Item>
  <Item name="sourceType">
    <p>
      What changed. Could be a store we're subscribed to via{' '}
      <Link to="../injectors/injectStore">
        <code>injectStore()</code>
      </Link>
      , or an atom instance that we injected, or an{' '}
