...
When called synchronously during atom or selector evaluation, `get`, `getInstance`, and `select` will register graph dependencies on resolved [atom instances](../classes/AtomInstance) and [SelectorCaches](../classes/SelectorCache). In this way, `get` is like [`injectAtomValue`](../injectors/injectAtomValue), `getInstance` is like [`injectAtomInstance`](../injectors/injectAtomInstance), and `select` is like [`injectAtomSelector`](../injectors/injectAtomSelector).
...
<Legend>
  <Item name="ecosystem">
    <p>
      ...
      atom or selector was created in.
    </p>
    <p>
      Note that while we call ecosystems themselves AtomGetters, they are the
      only AtomGetters that don't have an <code>ecosystem</code> property (they
...
ecosystem.findAll(myAtom, [param1, param2])`}</Ts>
  ...
  <Item name="get">
    <p>
      ...
      Pass either an instance directly or an atom template and its params (if
      the atom takes params).
    </p>
    <Ts>{`value = get(myAtom)
...
const val = instance ? get(instance) : defaultVal`}</Ts>
  ...
  <Item name="getInstance">
    <p>
      ...
      params combo. Pass either an instance directly or an atom template and its
      params (if the atom takes params).
    </p>
