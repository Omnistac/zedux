...
value = select(atomSelectorCache)`}</Ts>
    ...
    <p>
      If a SelectorCacheItem is passed, <code>select()</code> registers a
    ...
    <p>
      Use{' '}
      <Link to="../classes/SelectorCache#find">
        ecosystem.selectorCache.find(mySelector)?.result
      </Link>{' '}
    ...
    <Ts>{`const cache = ecosystem.selectorCache.find(mySelector)
