...
A function that runs an [AtomSelector](AtomSelector). Also accepts an [AtomSelectorConfig object](AtomSelector#atomselectorconfig) with the AtomSelector as the `selector` property.

```ts
select(atomSelectorOrConfig, ...args?) => result
```

When called synchronously during atom instance or AtomSelector evaluation, `select` registers [restricted dynamic graph dependencies](../glossary/#restricted-dynamic-graph-dependency) on any atom instances The passed AtomSelector gets.

`select` will register static dependencies for every `getInstance`, dynamic dependencies for every `get`, and restricted dynamic dependencies for every `select` that it calls (this is recursive!) synchronously.
