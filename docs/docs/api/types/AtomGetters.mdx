...
const instance = maybeInstance ? getInstance(maybeInstance) : undefined`}</Ts>
  ...
  <Item name="select">
    <p>
      ...
      <Link to="../classes/SelectorCacheInstance">SelectorCacheInstance</Link>{' '}
      instance.
    </p>
...
value = select(atomSelectorCache)`}</Ts>
    ...
    <p>
      If an SelectorCacheInstance is passed, <code>select()</code> registers a
...
const val = cache ? select(cache) : defaultVal`}</Ts>
    ...
    <p>
      ...
      <Link to="../classes/SelectorCacheInstance#cachekey">
        <code>cacheKey</code>
      </Link>
      .
    </p>
