...
When called synchronously during atom instance or AtomSelector evaluation, `get`, `getInstance`, and `select` will register graph dependencies on resolved atom instances. In this way, `get` is like [`injectAtomValue`](../injectors/injectAtomValue), `getInstance` is like [`injectAtomInstance`](../injectors/injectAtomInstance), and `select` is like [`injectAtomSelector`](../injectors/injectAtomSelector).
...
When called asynchronously, AtomGetters don't register graph dependencies. Thus `get` becomes an alias for `ecosystem.get()`, `getInstance` an alias for `ecosystem.getInstance()`, and `select` an alias for `ecosystem.select()`. They simply return the expected values.
...
<Legend>
  <Item name="ecosystem">
    <p>
      A reference to the <Link to="../classes/Ecosystem">ecosystem</Link> this
      atom/ion instance or AtomSelector was created in.
    </p>
    <p>
      Note that while we call ecosystems AtomGetters, they are the only
      AtomGetters that don't have an <code>ecosystem</code> property (they don't
      need it obviously)
    </p>
    <p>
...
const staticVal = ecosystem.get(val) // doesn't register anything`}</Ts>
    <p>The ecosystem also has additional getters:</p>
    <Ts>{`// returns an atom instance only if one exists already
...
ecosystem.weakGet(myAtom, [param1, param2])`}</Ts>
    <p>
      On the ecosystem, you also have access to the{' '}
      <Link to="../classes/Ecosystem#selectorcache">
        <code>.selectorCache</code>
      </Link>{' '}
      property for full control over manually controlling cached selectors.
    </p>
  </Item>
  <Item name="get">
    <p>
      A function that returns the current state of an atom instance's store.
      Pass either an instance directly or an atom and its params (if the atom
      takes params).
    </p>
    <Ts>{`value = get(myAtom)
...
value = get(myAtomInstance)`}</Ts>
    ...
    <p>
      When an atom is passed, <code>get()</code> creates the instance if it
      doesn't exist yet, registers a dynamic graph dependency on the new
    ...
    <p>
      ...
      dependencies. Other than that, it behaves the same.
    </p>
    <p>
      Use{' '}
      <Link to="../classes/Ecosystem#weakget">
        <code>ecosystem.weakGet()</code>
      </Link>{' '}
      to get a value only if the instance already exists. Note however that the
      ecosystem's getter functions don't register graph dependencies. To weakly
      register a dependency, use the following pattern:
    </p>
    <Ts>{`const instance = ecosystem.weakGetInstance(myAtom, [param1, param2])
const val = instance ? get(instance) : defaultVal`}</Ts>
  </Item>
  <Item name="getInstance">
    <p>
      A function that returns the atom instance for the given atom + params
      combo. Pass either an instance directly or an atom and its params (if the
      atom takes params).
    </p>
...
instance = getInstance(myAtomInstance)`}</Ts>
    ...
    <p>
      When an atom is passed, <code>get()</code> creates the instance if it
      doesn't exist yet, registers a static graph dependency on the new
    ...
    <p>
      Use{' '}
      <Link to="../classes/Ecosystem#weakgetinstance">
        <code>ecosystem.weakGetInstance()</code>
      </Link>{' '}
      to get an instance only if it already exists. Note however that the
      ecosystem's getter functions don't register graph dependencies. To weakly
      register a static dependency, use the following pattern:
    </p>
    <Ts>{`const maybeInstance = ecosystem.weakGetInstance(myAtom, [param1, param2])
const instance = maybeInstance ? getInstance(maybeInstance) : undefined`}</Ts>
  ...
  <Item name="select">
    <p>
      ...
      <Link to="AtomSelectorConfig">AtomSelectorConfig</Link> object, or an{' '}
      <Link to="../classes/SelectorCacheItem">SelectorCacheItem</Link> instance.
    </p>
    <Ts>{`value = select(atomSelector, arg1, arg2)
value = select(atomSelectorConfig, arg1, arg2)
value = select(atomSelectorCache)`}</Ts>
    <p>
    ...
    <p>
      If an SelectorCacheItem is passed, <code>select()</code> registers a
      dynamic dependency on the cache and returns the cache's current result.
    </p>
    <p>
      When called asynchronously (after the current atom instance or selector
    ...
    <p>
      Use{' '}
      <Link to="../classes/SelectorCache#weakgetcache">
        ecosystem.selectorCache.weakGetCache()?.result
      </Link>{' '}
    ...
    <Ts>{`const cache = ecosystem.selectorCache.weakGetCache(mySelector)
const val = cache ? select(cache) : defaultVal`}</Ts>
    ...
    <p>
      ...
      <Link to="../classes/SelectorCacheItem#cachekey">
        <code>cacheKey</code>
      </Link>
      .
    </p>
