...
function MyComponent() {
  ...
  const dispatch = myFirstAtom.useDispatch()
...
const secondsAtom = atom('seconds', () => {
  const [state, setState, store] = injectState(0)\n
  injectEffect(() => {
...
function Seconds() {
  const state = secondsAtom.useValue()\n
...
An optional `<AppProvider />` can be rendered at the top level of your app. All atom instances created in any children will be scoped to the AppProvider.
...
See [the Dependency Injection tutorial](TODO:link) and [`<AppProvider />`](TODO:link)
...
See [the stores tutorial](TODO:link), [`createActor()`](TODO:link), [`createReducer()`](TODO:link), and [`createStore()`](TODO:link).
...
:::note
🚧 `maxInstances` is under construction. (`ttl` is good to go though)
:::
...
See [`injectInvalidate()`](TODO:link) and [the cache management tutorial](TODO:link)
...
See [`query()`](TODO:link), [`asyncMachine`](TODO:link), and [the side effects tutorial](TODO:link).
...
:::note
🚧 `injectSuspender` is under construction
:::
...
function App() {
  ...
  return (
    <Provider store={store}>
      <AppProvider contexts={[reduxInstance]}>
        <Routes />
      </AppProvider>
    </Provider>
  )
...
:::note
🚧 `injectAllInstances` is under construction
:::
...
See [the persistence tutorial](TODO:link).
...
:::note
🚧 `injectWhy` is not fully tested. It may not work in some cases. It is safe to use though.
:::
