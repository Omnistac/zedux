...
Data can be provided from an app's components layer to the atoms layer with app context.

```tsx
import { AppProvider, atom, injectAppContext } from '@zedux/react'

const someAtom = atom('some', () => {
  const context = injectAppContext()
})

function App() {
  return (
    <AppProvider context={{ anyValues: 'here' }}>
      <Routes />
    </AppProvider>
  )
}
