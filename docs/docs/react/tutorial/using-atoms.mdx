...
const userAtom = atom('user', () => ({ name: 'Joe', lastUpdated: new Date() }))\n
function Child() {
  ...
  return (
    <>
      ...
      <button onClick={() => setState({ lastUpdated: new Date() })}>
        Update Date
      </button>
    </>
  )
...
Returns a function that can be used to dynamically create atom instances. Often used in conjunction with AppProvider's [`preload`](TODO:link) prop:
