...
Calls an effect handler every time the store's state changes in response to an action.
...
To perform a side effect on every action or every state change while the predicate is matched, use `.receivesAction()` or `.stateChanges()` with a normal if statement:
...
const buttonPressAtom = atom('buttonPress', () => {
  // We use injectMemo() instead of injectStore() since we don't need this
  // component to reevaluate when the store's state changes.
