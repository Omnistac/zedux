...
`atom.injectState()` is just like `atom.useState()`, but is used in atoms instead of components. This is a common theme with injectors:
...
This example creates two instances of `userDataAtom` - one that fetches the `joe` user and one that fetches `bob`.
