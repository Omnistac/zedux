...
Zedux stores do not have middleware. Read more on that in the [Redux comparison](/not-done). Basic side effects like logging and state persistence can be done by hooking into a store's side effects layer.
...
const alertAtom = atom('alert', () => {
  ...
  const store = injectStore(0, true) // subscribe to the store\n
  injectEffect(() => {
