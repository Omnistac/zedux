...
As a general rule, all [atom params](../api/classes/AtomInstanceBase#params) must be serializable. There is one exception: You can pass an atom instance to another atom instance.

This is possible because all atom instances already have a unique, serialized [`keyHash`](../api/classes/AtomInstanceBase#keyhash) that will simply be appended to the receiving atom's `keyHash`.
...
This is another form of Dependency Injection.
...
```ts
const myAtom = atom('my', 'some state')
const myAtomOverride = atom('my', 'overridden state!', { ttl: 0 })
```
