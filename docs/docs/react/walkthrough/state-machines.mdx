...
Actors can be used directly, instead of using string constants
...
const resolved = createState('resolved')
...
const themeAtom = atom('theme', () => {
  const store = injectStore(() => createStore(machine))\n
  return store
})\n
const getStyle = (theme: string) => {
  switch (theme) {
    ...
    case dark.type:
      return { background: '#333542', color: '#fff' }
    case classic.type:
      return { background: '#fed', borderRadius: '20px', fontStyle: 'italic' }
  }
}\n
const buttonStyle = {
  appearance: 'none',
...
function Theme() {
  ...
  const { dispatch } = useAtomInstance(themeAtom)\n
  return (
    <button
      onClick={() => dispatch(cycle())}
      style={{ ...buttonStyle, ...getStyle(theme) }}
    >Cycle Theme</button>
  )
}
`}</LiveEditor>
