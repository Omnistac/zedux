...
In the quick start, we learned a few basic ways to create atoms. In this page, we'll create some more advanced atoms using config objects, injectors, stores, and the AtomApi. We'll also discover some specialized types of atoms.
...
:::note
🚧 `maxInstances` is under construction
:::

Complementary to `ttl`, `maxInstances` specifies the largest ideal cache size. If more instances than this "maximum" are created, Zedux will start cleaning them up automatically when they go stale, regardless of `ttl`.
...
injectEffect(() => {
  fetch('/api/blog-posts').then(async data => {
...
:::note
🚧 Suspense support is under construction
:::
...
Now you may be asking how one is to acquire a stable promise reference without kicking off side effects in the evaluator body. Or what if one wants to forward promises from dependency atoms on to React. Or what if one wants more control, more power, more complete and even utter world domination??

😒 We'll cover much more about suspense in [the suspense walkthrough](/not-done).

AtomApis have many more features that we'll cover eventually. Or check out the [AtomApi reference](/not-done) for all the power.
...
```tsx
import { AtomInstanceProvider, localAtom, useAtomInstance } from '@zedux/react'
...
function Child() {
  ...
  const instance = useAtomContext(reactContextAtom)
  ...
}
```
...
- All atom instances have an underlying store. This store can be created manually.
- `injectStore` creates a stable store reference.
- `injectEffect` runs side effects
- `injectMemo` memoizes values
- `injectGet` returns a utility function for retrieving atom instance values dynamically.
