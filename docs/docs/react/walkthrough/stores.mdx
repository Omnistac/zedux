...
:::info
Zedux actions must have a string `type` property and can have optional `payload` and `meta` properties.
:::
...
Atoms create their own stores by default every time an instance is created. However, sometimes the atom needs access to the instance's store during evaluation. Zedux provides several injectors that can be used to create a store in an evaluator.
...
You can set a store's state during evaluation.
...
:::tip
Both `store.setState()` and `store.dispatch()` run completely synchronously. After calling these, you can always know that the store's state has been updated.
:::

`store.setState()` and `store.dispatch()` return the new state. We could rewrite the above example to make use of this:
