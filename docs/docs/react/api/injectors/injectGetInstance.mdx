...
This overload is mostly just for interop in cases where you have a value that may be either an atom or instance (Zedux does this internally). However, it can be useful for purposefully registering a static dependency on an instance when the instance was [passed via params](../classes/AtomInstanceBase#params).
...
Required. An [atom](../classes/AtomBase).
...
Required (in this overload). An [atom instance](../classes/AtomInstanceBase). The same atom instance will be returned. When called synchronously during evaluation, this will register a static dependency on the passed atom instance (usually not necessary).
