...
If an object is passed, Zedux will spider through it and create a reducer hierarchy. Zedux will then call this reducer to get the initial state of the store.

If an initialState is also passed, Zedux will set that as the state before calling the root reducer.

### `initialState`

The initial state of the store. Stores can hold anything. If using the [zero-config pattern](/not-done), pass `null` as the initialHierarchy.
