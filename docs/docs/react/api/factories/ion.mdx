...
import { AtomKeyDesc } from '../classes/AtomBase.mdx'

```ts
import { ion } from '@zedux/react'
```

`ion()` is a factory for creating [Ions](../classes/Ion). Ions are a type of atom that specialize in selection/derivation operations.
...
The first parameter passed to `get` will be a special [IonGetUtils object](../types/IonGetUtils). Any parameters after that will be the actual params of the atom instance.
...
Optional. A function that receives a special [IonSetUtils object](../types/IonSetUtils) and returns either void or the new state of this ion atom instance.
...
- Params - the tuple of parameters the evaluator function accepts. Must be an array or tuple type. These are the parameters _after_ the [utils object](../types/IonGetUtils).
