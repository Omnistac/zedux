...
Local atoms expose only one method apart from those inherited from [`StandardAtomBase`](StandardAtomBase):

### `.override`

Creates an exact clone of this local atom, but with a different value.
