...
<AtomInstanceProvider instance={instance}>{children}</AtomInstanceProvider>
```

## Signature

```ts
injectAtomInstance(atom, params?) => AtomInstance
```
...
```ts
injectAtomInstance(instance) => AtomInstance
```
