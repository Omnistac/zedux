...
A React hook accepts an [atom](../classes/AtomBase) and returns an [instance](../classes/AtomInstanceBase) of that atom that has been provided over React context via [`<AtomInstanceProvider>`](../components/AtomInstanceProvider).
...
Optional. A boolean. If true, Zedux will throw an error if you forgot to provide an atom instance.
