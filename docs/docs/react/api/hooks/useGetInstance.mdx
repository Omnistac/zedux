...
Unlike the `getInstance` function returned from [`injectGetInstance()`](../injectors/injectGetInstance), this `getInstance` function never registers any graph dependencies.
...
Note that unlike [`injectGetInstance()`](../injectors/injectGetInstance), there is no overload for passing an atom instance.
