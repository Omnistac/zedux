...
const smallListAtom = selector('smallList', () => {
  const bigList = injectAtomValue(bigListAtom)
...
function Counter() {
  ...
  const { decrement, increment } = useAtomInstance(counterAtom).methods
...
See [the stores walkthrough](../walkthrough/stores) and [the `Store` class](../classes/Store).
...
See [`query()`](/not-done), [`asyncMachine`](/not-done), and [the side effects walkthrough](../walkthrough/side-effects).
