...
<LiveEditor ecosystemId="overview/injectors" resultVar="Seconds">{`
...
```tsx
import { api, ion } from '@zedux/react'
...
<LiveEditor ecosystemId="overview/stores" resultVar="BigComponent">{`
...
:::note
🚧 The `@zedux/react/query` package is under construction
:::

React Query-style queries are just another type of atom:

```tsx
import { query, useQuery } from '@zedux/react/query'
...
See [`query()`](/not-done) and [the side effects walkthrough](../walkthrough/side-effects).
...
```tsx
import { mutation, useMutation } from '@zedux/react/query'

const updateUserMutation = mutation('updateUser', () => {
  const API_URL = injectAtomSelector(({ get }) => get(configAtom).API_URL)
...
:::note
🚧 Molecules and `injectAllInstances` are under construction
:::
...
:::note
🚧 `molecule` and `injectAllInstances` is under construction
:::

Hydrating initial atom state can be done with molecules.
...
Atoms (including molecules) can be preloaded to kickstart hydration, alleviate render waterfalls, and bootstrap data from outside Zedux.

```tsx
import { EcosystemProvider, useGetInstance } from '@zedux/react'
...
:::note
🚧 The `@zedux/react/state-hub` package is under construction
:::
