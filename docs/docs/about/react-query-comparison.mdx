...
- For tracking promise state: Zedux has query atoms and [`injectPromise()`]. Queries track promise state by default.
...
This is more flexible, but does require more boilerplate. Most functionality can be abstracted, of course. We may provide a `@zedux/query` package someday that provides a full suite of tools for handling promises.
