...
- In Zedux, store dispatches are completely synchronous. In Redux, synchrony depends on your middleware.
...
Zedux - use `actionFactory()` and `createReducer()`. No immer (for now).
