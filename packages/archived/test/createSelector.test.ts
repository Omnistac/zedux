...
import { nonPlainObjects } from '@zedux/core-test/utils'
...
describe('Zedux.createSelector()', () => {
  ...
  test('returns a selector whose input selectors are all args but the last, and whose calculator function is the last arg', () => {
    ...
    const calculator = jest.fn()
    const memoizedSelector1 = createSelector(selector1, calculator)
    const memoizedSelector2 = createSelector(selector1, selector2, calculator)
